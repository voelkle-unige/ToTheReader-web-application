...
(:
 ...
 :)
declare variable $config:default-template :="TTR3.html";
...
declare variable $config:facets := [
    ...
    map {
        ...
        "output": function($label) {
            switch($label)
                case "X to the Y" return "Bibliographical Address"
                default return $label
        }
    },
    map {
        "dimension": "USTCsubcat",
        "heading": "USTC Subject",
        "max": 5,
        "hierarchical": false()
    },
    map {
...
(:~
 ...
 :)
declare variable $config:default-odd :="TTR2.odd";
...
(:~
 ...
 :)
declare variable $config:odd-available :=("TTR2.odd");
